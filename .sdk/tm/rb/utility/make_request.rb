...
module ArulsIpUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ArulsIpResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArulsIpResponse.new(fetched)
    else
