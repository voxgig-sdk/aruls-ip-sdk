...
class FtCtrl extends ArulsIpControl
{
...
class FtClient
{
    ...
    private ArulsIpUtility $utility;

    public function __construct(ArulsIpUtility $utility, array $options)
    {
    ...
    public function get_utility(): ArulsIpUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ArulsIpContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ArulsIpContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ArulsIpConfig::make_feature($name);
    ...
    private static function buildUrl(ArulsIpSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ArulsIpError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ArulsIpResponse($fetched);
        }
    ...
    private function fail(ArulsIpContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ArulsIpContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
