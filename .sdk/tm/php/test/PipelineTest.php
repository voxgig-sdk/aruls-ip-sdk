...
{
    private static function utility(): ArulsIpUtility
    {
        return new ArulsIpUtility();
    }
    ...
    private static function ctx(array $over = []): ArulsIpContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ArulsIpContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ArulsIpOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ArulsIpError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ArulsIpOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ArulsIpMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ArulsIpOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ArulsIpMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ArulsIpOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ArulsIpMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArulsIpSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ArulsIpMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ArulsIpResponse([]),
            'result' => new ArulsIpResult([]),
        ]);
        [, $err] = ArulsIpMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ArulsIpSpec([]),
            'response' => null,
            'result' => new ArulsIpResult([]),
        ]);
        [, $err] = ArulsIpMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ArulsIpSpec(['step' => 's']),
            'response' => new ArulsIpResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ArulsIpResult([]),
        ]);
        [, $err] = ArulsIpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ArulsIpSpec(['step' => 's']),
            'response' => new ArulsIpResponse(self::resp(200, ['v' => 1])),
            'result' => new ArulsIpResult([]),
        ]);
        [, $err] = ArulsIpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ArulsIpSpec(['step' => 's']),
            'response' => new ArulsIpResponse(self::resp(200, ['v' => 2])),
            'result' => new ArulsIpResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ArulsIpMakeResponse::call($ctx);
    ...
    {
        $preset = new ArulsIpResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ArulsIpSpec([]),
            'response' => new ArulsIpResponse([]),
            'result' => new ArulsIpResult([]),
        ...
        [$got, $err] = ArulsIpMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ArulsIpResult([])]);
        [, $err] = ArulsIpMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ArulsIpSpec([]), 'result' => null]);
        [, $err] = ArulsIpMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArulsIpSpec(['step' => 's']),
            'result' => new ArulsIpResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ArulsIpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArulsIpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArulsIpSpec(['step' => 's']),
            'result' => new ArulsIpResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ArulsIpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArulsIpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArulsIpResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ArulsIpSpec([]),
            'result' => new ArulsIpResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ArulsIpMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArulsIpMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ArulsIpError('boom', 'boom');
        $utility->fetcher = function (ArulsIpContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArulsIpSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ArulsIpMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ArulsIpContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ArulsIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ArulsIpResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ArulsIpContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArulsIpSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ArulsIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ArulsIpResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ArulsIpSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ArulsIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ArulsIpMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ArulsIpMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ArulsIpResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ArulsIpDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ArulsIpResult(['ok' => false])]);
        $this->expectException(ArulsIpError::class);
        ArulsIpDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ArulsIpResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ArulsIpMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ArulsIpMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ArulsIpBaseFeature();
        $b = new ArulsIpBaseFeature();
        ArulsIpFeatureAdd::call($ctx, $a);
        ArulsIpFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ArulsIpBaseFeature
    {
        $f = new ArulsIpBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ArulsIpFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ArulsIpContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ArulsIpSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArulsIpPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ArulsIpResponse(['headers' => null]),
            'result' => new ArulsIpResult([]),
        ]);
        ArulsIpResultHeaders::call($ctx);
