...
ArulsIpSDK._make_feature = _make_feature


function ArulsIpSDK.new(options)
  local self = setmetatable({}, ArulsIpSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ArulsIpSDK:options_map()
...
function ArulsIpSDK:get_utility()
  return Utility.copy(self._utility)
end


function ArulsIpSDK:get_root_ctx()
  return self._rootctx
end


function ArulsIpSDK:prepare(fetchargs)
...
function ArulsIpSDK:direct(fetchargs)
...
-- by passing it.
function ArulsIpSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ArulsIpSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ArulsIpSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ArulsIpSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ArulsIpSDK.new(sdkopts)
  sdk.mode = "test"
