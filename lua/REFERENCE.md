# ArulsIp Lua SDK Reference

Complete API reference for the ArulsIp Lua SDK.
...
```lua
local result, err = client:IpAddress(nil):load({ id = "ip_address_id" }, nil)
```
...
```lua
local result, err = client:Ipn(nil):load({ id = "ipn_id" }, nil)
```
