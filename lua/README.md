...
The Lua SDK for the ArulsIp API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:IpAddress(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ArulsIp(nil):load(
  { id = "test01" }, nil
)
