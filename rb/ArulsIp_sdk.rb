...
require_relative 'ArulsIp_types'
...
class ArulsIpSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ArulsIpHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ArulsIpHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ArulsIpFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ArulsIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ArulsIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ArulsIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ArulsIpError.new(
        "#{op}_allow",
        "ArulsIpSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ArulsIpHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ArulsIpError.new(
        "graphql_error", "ArulsIpSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
