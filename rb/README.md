...
The Ruby SDK for the ArulsIp API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.IpAddress(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ArulsIp(nil).load(
  { "id" => "test01" }, nil
)
