...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ARULSIP_TEST_LIVE ||
    'TRUE' === process.env.ARULSIP_TEST_OVERRIDE
  ) {
  ...
  m.ARULSIP_TEST_EXPLAIN = process.env.ARULSIP_TEST_EXPLAIN || m.ARULSIP_TEST_EXPLAIN
