...
//     (\`new ArulsIpSDK(...)\` / \`ArulsIpSDK.test(...)\` become
//     \`ArulsIpSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"ip_address":{"test01":{"id":"test01"}},"ipn":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ArulsIpSDK(...)\` and any
// \`ArulsIpSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ArulsIpSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ArulsIpSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
