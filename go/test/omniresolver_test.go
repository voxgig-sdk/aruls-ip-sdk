...
import (
	...
	omni "github.com/voxgig-sdk/aruls-ip-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ArulsIpError); is {
				out := map[string]any{
					"name":    "ArulsIpError",
...
func omniCtx(arg any, client *sdk.ArulsIpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ArulsIpSDK); is {
			client = live
