...
class ArulsIpSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ArulsIpHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ArulsIpHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ArulsIpFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ArulsIpFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ArulsIpUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ArulsIpHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ArulsIpHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ArulsIpHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ArulsIpError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ArulsIpHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ArulsIpError("graphql_error",
                "ArulsIpSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
