...
class IpAddressEntity
{
    ...
    {
        if ($args) {
            $this->_data = ArulsIpHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = ArulsIpHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param IpAddressLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IpAddressLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return IpAddress|array The loaded IpAddress as an assoc-array at the
     *   SDK boundary; throws ArulsIpError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ArulsIpHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
