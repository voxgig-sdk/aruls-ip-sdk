...
MCP server exposing the ArulsIp SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `aruls-ip_list` | `entity`, optional `query` map | First page of records as JSON |
| `aruls-ip_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `aruls-ip_list` and
`aruls-ip_load` tools become available in new sessions.
