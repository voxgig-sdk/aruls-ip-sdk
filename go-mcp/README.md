...
[MCP](https://modelcontextprotocol.io) server exposing the ArulsIp SDK as
two agent tools — `aruls-ip_list` and `aruls-ip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/aruls-ip-mcp)
make build

# 2. Provide credentials via the environment
export ARULS_IP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user aruls-ip \
  -- /absolute/path/to/aruls-ip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./aruls-ip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// aruls-ip_list: first page of records
{ "entity": "ip_address" }
{ "entity": "ip_address", "query": { } }

// aruls-ip_load: one record by id
{ "entity": "ip_address", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/aruls-ip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user aruls-ip \
     -- "$PWD"/dist/*/aruls-ip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `aruls-ip_list` and `aruls-ip_load` tools now appear
   in new sessions. Ask the agent to *"list ip_address using aruls-ip"*
   and it calls `aruls-ip_list` with `{"entity":"ip_address"}`.
...
```sh
export ARULS_IP_APIKEY=sk_live_xxx            # API key
export ARULS_IP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `aruls-ip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `aruls-ip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ARULS_IP_APIKEY` | API key sent with every request. |
| `ARULS_IP_BASE` | Optional override of the API base URL. |
...
```sh
./aruls-ip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"aruls-ip_load","arguments":{"entity":"ip_address","query":{"id":1}}}}'
```
