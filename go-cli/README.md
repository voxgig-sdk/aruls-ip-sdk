...
boru-driven command-line client **and** interactive REPL for the ArulsIp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/aruls-ip-cli)
make build
...
./aruls-ip-cli load 1 ip_address            # {id:1} shorthand
./aruls-ip-cli load '{id:1}' ip_address       # explicit match map

# 5. Override the API base URL for a single call
ARULS_IP_BASE=https://api.example.com ./aruls-ip-cli load 1 ip_address

# 6. No arguments -> interactive REPL
./aruls-ip-cli
aruls-ip> load 1 ip_address
aruls-ip> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/aruls-ip-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/aruls-ip-cli load 1 ip_address
   ```
...
```sh
./aruls-ip-cli load 1 ip_address          # scalar shorthand for {id:1}
./aruls-ip-cli load '{id:1}' ip_address     # explicit match map
```
...
./aruls-ip-cli load 1 ip_address
```
...
Run with no arguments to open a REPL (prompt `aruls-ip>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ip_address`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/aruls-ip-cli`. |
...
(`./aruls-ip-cli <expr>`) and interactively (the REPL), and expressions compose the
