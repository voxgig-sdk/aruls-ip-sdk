// Command ArulsIp-cli is a CLI + REPL for the ArulsIp SDK,
...
const prompt = "aruls-ip"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ip_address ipn"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ARULS_IP_APIKEY (api key), ARULS_IP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ARULS_IP_APIKEY carries the API key and
	// ARULS_IP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ARULS_IP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ARULS_IP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewArulsIpSDK(opts)
