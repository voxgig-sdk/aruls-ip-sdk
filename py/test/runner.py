# ArulsIp SDK test runner

from __future__ import annotations
import os
import json

from arulsip_sdk.utility.voxgig_struct import voxgig_struct as vs


class ArulsIpTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ArulsIpTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ArulsIpTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ArulsIpTestRunner.getenv("ARULS_IP_TEST_LIVE")
        override = ArulsIpTestRunner.getenv("ARULS_IP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ArulsIpTestRunner.getenv(key)
        ...
        explain = ArulsIpTestRunner.getenv("ARULS_IP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ARULS_IP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ArulsIpTestRunner._test_control is not None:
            return ArulsIpTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ArulsIpTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ArulsIpTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ArulsIpTestRunner.load_test_control()
...
def load_env_local():
    ArulsIpTestRunner.load_env_local()


def env_override(m):
    return ArulsIpTestRunner.env_override(m)


def entity_data(v):
    return ArulsIpTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ArulsIpTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ArulsIpTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ArulsIpTestRunner.load_test_control()


def live_client_options():
    return ArulsIpTestRunner.live_client_options()


def live_delay_ms():
    return ArulsIpTestRunner.live_delay_ms()
