...
The Python SDK for the ArulsIp API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.IpAddress(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ArulsIp(None).load(
    {"id": "test01"}, None
)
