# ArulsIp Python SDK Reference

Complete API reference for the ArulsIp Python SDK.
...
#### `ArulsIpSDK.test(testopts=None, sdkopts=None)`
...
```python
result, err = client.IpAddress().load({"id": "ip_address_id"})
```
...
```python
result, err = client.Ipn().load({"id": "ipn_id"})
```
